...
<head>
    ...
    <title>Pagamentos Visitantes - Nosso Zimbo</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <button class="nav-item active" onclick="window.location.href='admin_pagamentos_visitantes.php'">
...
<main class="main-content">
    <header class="topbar">
        ...
        <span class="topbar-title">👥 Controlo de Acessos de Visitantes</span>
    ...
    <div style="padding: 2.5rem 3rem;">
        <div class="page-header">
            <h1 class="page-title">Pagamentos e Visitas</h1>
            <p class="page-sub">Monitore quem entra e sai do condomínio</p>
        </div>

        <div class="card">
            <div class="card-head"><p class="card-title"><i class="fa-solid fa-id-card-clip"></i> Registos de Visitas</p></div>
            <div style="overflow-x:auto;">
                <table class="data-table">
                    ...
                    <tbody id="visitas-tbody">
                        <tr><td colspan="8" style="text-align:center;">Carregando registos de acesso...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
async function loadVisitas() {
    try {
        ...
        tbody.innerHTML = data.dados.map(v => `
            <tr>
                <td><strong>${v.codigo_acesso || v.id}</strong></td>
                <td>${v.nome_visitante}</td>
                <td>${v.morador} (${v.apartamento})</td>
                <td>0 Kz</td>
                <td>N/A</td>
                <td>${new Date(v.data_prevista).toLocaleDateString()} ${v.hora_prevista}</td>
                <td><span class="badge ${v.estado === 'confirmado' ? 'pago' : 'pendente'}">${v.estado}</span></td>
                <td>
                    <button class="btn-secondary btn-sm" onclick="confirmarVisita(${v.id})"><i class="fa-solid fa-check"></i> Autorizar</button>
                </td>
            </tr>
        `).join('');
    } catch(e) {}
...
async function confirmarVisita(id) {
    ...
    fd.append('id', id);
    fd.append('tipo', 'visita');
    fd.append('estado', 'confirmado');
    const r = await fetch(`${API_URL}?acao=validar_agendamento`, { method:'POST', body:fd });
    if((await r.json()).sucesso) { showToast('Visita Autorizada!'); loadVisitas(); }
}
